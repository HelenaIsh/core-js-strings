...
function sumOfCodes(str) {
  return str
    ? str.split('').reduce((acc, val, i) => acc + str.charCodeAt(i), 0)
    : 0;
}
  ...
  return str
    ...
    .map((char) => {
      if (alphabet.includes(char.toLowerCase())) {
        ...
        const temp = alphabet.indexOf(char.toLowerCase()) + 13;
        const index = temp >= alphabet.length ? temp - alphabet.length : temp;
        return isLowerCase ? alphabet[index] : alphabet[index].toUpperCase();
      }
      return char;
    })
    .join('');
